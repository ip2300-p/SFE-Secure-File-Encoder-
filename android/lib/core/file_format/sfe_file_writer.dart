...
class SfeFileWriter {
  ...
  static Future<void> writeHeader(
    ...
    // رمزنگاری اسم فایل + حجمش (با یه بایت null بینشون جدا میشه)
    // فرمت: "اسم‌فایل\x00حجم" - اگه originalFileSize ندیم، دقیقاً مثل قبل
    // فقط اسم فایل نوشته میشه (سازگار با فایل‌های قدیمی‌تر)
    final filename = header.originalFilename ?? 'unknown';
    final metaString = header.originalFileSize != null
        ? '$filename\u0000${header.originalFileSize}'
        : filename;
    final filenameBytes = utf8.encode(metaString);
